...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action'])) {
    if ($_POST['action'] === 'take_token') {
        ...
        if ($tokenId > 0) {
            ...
            logAudit($pdo, 'take_service_token', 'service_token', $tokenId, 'Employee took ownership of the token');
        ...
        if ($currentToken) {
            ...
            if ($assignedTo && (int)$currentToken['assigned_to'] !== $assignedTo) {
                notify($pdo, $assignedTo, 'Service token assigned', 'A marketplace service token has been assigned to you.', 'task', '/employee/service_tokens.php');
            }
...
<form method="GET" class="glass-card p-4 rounded-2xl mb-6 flex flex-wrap gap-3 items-end">
    ...
    <a href="/employee/service_tokens.php" class="btn-secondary px-4 py-2 rounded-lg text-sm text-slate-300">Clear</a>
...
Thank you for contacting Electava. We have reviewed your service request and our team is working on the next steps.

Please let us know if there are any additional files, notes, or requirements you would like us to review.

Best regards,
Electava Team</textarea>
                    </div>
