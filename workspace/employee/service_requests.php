...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action'])) {
    if ($_POST['action'] === 'take_request') {
        ...
        if ($requestId > 0) {
            ...
            logAudit($pdo, 'take_service_request', 'service_request', $requestId, 'Employee took ownership of the request');
        ...
        if ($currentRequest) {
            ...
            if ($assignedTo && (int)$currentRequest['assigned_to'] !== $assignedTo) {
                notify($pdo, $assignedTo, 'Service request assigned', 'A service request has been assigned to you.', 'task', '/employee/service_requests.php');
            }
...
<form method="GET" class="glass-card p-4 rounded-2xl mb-6 flex flex-wrap gap-3 items-end">
    ...
    <a href="/employee/service_requests.php" class="btn-secondary px-4 py-2 rounded-lg text-sm text-slate-300">Clear</a>
...
<div class="space-y-4">
    ...
    <div class="glass-card rounded-2xl p-12 text-center border border-dashed border-slate-700/60">
        ...
        <p class="text-sm text-slate-500 mt-2">When customers create service requests, they will appear here for employees to pick up.</p>
    </div>
    <?php endif; ?>
</div>
